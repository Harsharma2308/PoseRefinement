...
for i = 1:numel(keyframes)
    ...
    filename = sprintf('../experiments/eval_result/ycb/Densefusion_iterative_result/%04d.mat', i - 1);
    result_my = load(filename);
    filename = sprintf('../experiments/eval_result/ycb/Densefusion_wo_refine_result/%04d.mat', i - 1);
    result_mygt = load(filename);
    filename = sprintf('../experiments/eval_result/ycb/Densefusion_iterative_result/%04d_icp.mat', i - 1);
    result_my_icp = load(filename);
    filename = sprintf('../experiments/eval_result/ycb/Densefusion_iterative_result/%04d_ICPi.mat', i - 1);
    result_my_icp2 = load(filename);
    filename = sprintf('../experiments/eval_result/ycb/Densefusion_iterative_result/%04d_ICPfor.mat', i - 1);
    result_my_icpfor = load(filename);
    filename = sprintf('../experiments/eval_result/ycb/Densefusion_iterative_result/%04d_ICPrev.mat', i - 1);
    result_my_icprev = load(filename);
    filename = sprintf('../experiments/eval_result/ycb/Densefusion_iterative_result/%04d_ICPth.mat', i - 1);
    result_my_icpth = load(filename);
    ...
    for j = 1:numel(gt.cls_indexes)
        ...
        if isempty(roi_index) == 0
            ...
            % pose after ICPi
            RT(1:3, 1:3) = quat2rotm(result_my_icp2.poses(roi_index, 1:4));
            RT(:, 4) = result_my_icp2.poses(roi_index, 5:7);
...
% 
             ...
             % pose from icp
             RT(1:3, 1:3) = quat2rotm(result_my_icp.poses(roi_index, 1:4));
             RT(:, 4) = result_my_icp.poses(roi_index, 5:7);
             ...
             % pose from icpfor
             RT(1:3, 1:3) = quat2rotm(result_my_icpfor.poses(roi_index, 1:4));
             RT(:, 4) = result_my_icpfor.poses(roi_index, 5:7);
             ...
             % pose from icprev
             RT(1:3, 1:3) = quat2rotm(result_my_icprev.poses(roi_index, 1:4));
             RT(:, 4) = result_my_icprev.poses(roi_index, 5:7);
             ...
             % pose from icpth
             RT(1:3, 1:3) = quat2rotm(result_my_icpth.poses(roi_index, 1:4));
             RT(:, 4) = result_my_icpth.poses(roi_index, 5:7);
             distances_sys(count, 7) = adi(RT, RT_gt, models{cls_index}');
             distances_non(count, 7) = add(RT, RT_gt, models{cls_index}');
             errors_rotation(count, 7) = re(RT(1:3, 1:3), RT_gt(1:3, 1:3));
             errors_translation(count, 7) = te(RT(:, 4), RT_gt(:, 4));           
        else
            distances_sys(count, 1:7) = inf;
            distances_non(count, 1:7) = inf;
            errors_rotation(count, 1:7) = inf;
            errors_translation(count, 1:7) = inf;
        end
